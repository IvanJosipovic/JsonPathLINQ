...
public sealed class JsonPathTests
{
    ...
    public void EmptyRangeIsValid(JsonPathTestCase testCase) => RunTest(testCase, allowMissingKeys: true);

    [Theory]
    [MemberData(nameof(NestedRangesData))]
    public void NestedRanges(JsonPathTestCase testCase) => RunTest(testCase);

    [Theory]
    [MemberData(nameof(FilterPartialMatchesAllowMissingData))]
    public void FilterPartialMatchesAllowMissing(JsonPathTestCase testCase) => RunTest(testCase, allowMissingKeys: true);

    [Theory]
    [MemberData(nameof(FilterPartialMatchesStrictData))]
    public void FilterPartialMatchesStrict(JsonPathTestCase testCase) => RunTest(testCase);

    [Theory]
    [MemberData(nameof(NegativeIndexData))]
    public void NegativeIndex(JsonPathTestCase testCase) => RunTest(testCase);

    [Theory]
    [MemberData(nameof(RunningPodsJsonOutputData))]
    public void RunningPodsJsonOutput(JsonPathTestCase testCase) => RunTest(testCase, allowMissingKeys: true);

    [Theory]
    [MemberData(nameof(StepData))]
    public void Step(JsonPathTestCase testCase) => RunTest(testCase);
